...
import {
    ...
    Layout, Camera, Music, Youtube, Search
} from 'lucide-react';
...
const MediaDownloaderPage: React.FC = () => {
    ...
    const [cookieStatus, setCookieStatus] = useState<Record<string, {found: boolean; hasSession: boolean; expiry: number | null}> | null>(null);
    const [showCookieManager, setShowCookieManager] = useState(false);
    const [cookieManagerTab, setCookieManagerTab] = useState<'auto' | 'manual'>('auto');
    const [selectedBrowser, setSelectedBrowser] = useState<'chrome' | 'firefox' | 'edge'>('chrome');
    const [cookiePlatform, setCookiePlatform] = useState<'instagram' | 'facebook' | 'youtube' | 'tiktok'>('instagram');
    const [manualCookiesText, setManualCookiesText] = useState('');
    ...
    const fetchCookieStatus = useCallback(async () => {
        try {
            const resp = await api.get('/media/cookies/status');
            if (resp.data.success) setCookieStatus(resp.data.platforms);
        } catch {}
    }, []);

    const handleUpdateFromBrowser = async () => {
        setCookieUpdating(true); setCookieMsg(null);
        try {
            const resp = await api.post('/media/cookies/update-from-browser', { browser: selectedBrowser });
            if (resp.data.success) {
                setCookieMsg({ type: 'ok', text: resp.data.message || 'Cookies atualizados!' });
                await fetchCookieStatus();
            } else {
                setCookieMsg({ type: 'err', text: resp.data.error || 'Falha ao extrair cookies.' });
            }
        } catch (e: any) {
            setCookieMsg({ type: 'err', text: e.response?.data?.error || e.message || 'Erro ao extrair cookies.' });
        } finally {
    ...
    const handleUpdateManual = async () => {
        if (!manualCookiesText.trim()) return;
        setCookieUpdating(true); setCookieMsg(null);
        try {
            const resp = await api.post('/media/cookies/update-manual', { cookiesText: manualCookiesText, platform: cookiePlatform });
            if (resp.data.success) {
                setCookieMsg({ type: 'ok', text: resp.data.message || 'Cookies salvos!' });
                setManualCookiesText('');
                await fetchCookieStatus();
            } else {
                setCookieMsg({ type: 'err', text: resp.data.error || 'Formato inválido.' });
            }
        } catch (e: any) {
            setCookieMsg({ type: 'err', text: e.response?.data?.error || e.message || 'Erro ao salvar cookies.' });
        } finally {
            setCookieUpdating(false);
        }
    };
    ...
    useEffect(() => {
        ...
        init();
    }, [fetchAccounts, fetchSchedule, fetchAssociations, fetchCookieStatus]);
    ...
    const getCookieStatusBadge = (platform: string) => {
        if (!cookieStatus) return null;
        const s = cookieStatus[platform];
        if (!s) return null;
        if (s.hasSession) return { color: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: '✅', label: 'OK' };
        if (s.found) return { color: 'bg-amber-100 text-amber-700 border-amber-200', icon: '⚠️', label: 'Sem sessão' };
        return { color: 'bg-red-100 text-red-600 border-red-200', icon: '❌', label: 'Ausente' };
    };

    const cookiePlatforms = ['instagram', 'facebook', 'youtube', 'tiktok'] as const;
    const hasCookieIssue = cookieStatus && cookiePlatforms.some(p => !cookieStatus[p]?.hasSession);

    const accSearchQuery = accountSearch.trim().toLowerCase();
    const matchesAccountSearch = (str?: string) => !accSearchQuery || (str ? str.toLowerCase().includes(accSearchQuery) : false);

    const filteredInstagram = accounts.instagram.filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));
    const filteredFacebook = accounts.facebook.filter(acc => matchesAccountSearch(acc.name));
    const filteredWhatsapp = accounts.whatsapp.filter(acc => matchesAccountSearch(acc.groupName) || matchesAccountSearch(acc.name));
    const filteredTelegram = accounts.telegram.filter(acc => matchesAccountSearch(acc.name));
    const filteredTwitter = accounts.twitter.filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));
    const filteredThreads = accounts.threads.filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));
    const filteredTiktok = (accounts.tiktok || []).filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));
    const filteredYoutube = (accounts.youtube || []).filter(acc => matchesAccountSearch(acc.channel_name) || matchesAccountSearch(acc.name));
    const filteredKwai = (accounts.kwai || []).filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));
    const filteredPinterest = (accounts.pinterest || []).filter(acc => matchesAccountSearch(acc.username) || matchesAccountSearch(acc.name));

    const totalFilteredAccounts = filteredInstagram.length + filteredFacebook.length + filteredWhatsapp.length +
        filteredTelegram.length + filteredTwitter.length + filteredThreads.length +
        filteredTiktok.length + filteredYoutube.length + filteredKwai.length + filteredPinterest.length;
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-20 px-4 sm:px-0 animate-fade-in">
            ...
            {/* 🍪 Cookie Manager */}
            <div className={`rounded-[20px] border-2 transition-all shadow-md ${
                hasCookieIssue ? 'border-amber-300 bg-amber-50' : 'border-gray-100 bg-white'
            }`}>
                <button
                    onClick={() => setShowCookieManager(v => !v)}
                    className="w-full flex items-center justify-between px-6 py-4 text-left"
                >
                    <div className="flex items-center gap-3">
                        <span className="text-2xl">{hasCookieIssue ? '⚠️' : '🍪'}</span>
                        <div>
                            <p className="font-black text-gray-800 text-sm">
                                {hasCookieIssue ? 'Cookies desatualizados — downloads podem falhar!' : 'Cookies de autenticação'}
                            </p>
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                                {cookiePlatforms.map(p => {
                                    const badge = getCookieStatusBadge(p);
                                    return badge ? (
                                        <span key={p} className={`text-xs font-bold px-2 py-0.5 rounded-full border ${badge.color}`}>
                                            {badge.icon} {p.charAt(0).toUpperCase() + p.slice(1)}: {badge.label}
                                        </span>
                                    ) : null;
                                })}
                                {!cookieStatus && <span className="text-xs text-gray-400 italic">Carregando status...</span>}
                            </div>
                        </div>
                    </div>
                    <span className={`text-gray-400 transition-transform ${showCookieManager ? 'rotate-180' : ''}`}>▼</span>
                </button>
                    ...
                    >
                        <div className="px-6 pb-6 space-y-4 border-t border-gray-100">
                            ...
                            <div className="flex gap-2 pt-4">
                                <button
                                    onClick={() => setCookieManagerTab('auto')}
                                    className={`flex-1 py-2.5 rounded-xl text-sm font-black transition-all ${
                                        cookieManagerTab === 'auto' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                    }`}
                                >
                                    🤖 Extração Automática
                                </button>
                                <button
                                    onClick={() => setCookieManagerTab('manual')}
                                    className={`flex-1 py-2.5 rounded-xl text-sm font-black transition-all ${
                                        cookieManagerTab === 'manual' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                    }`}
                                >
                                    ✍️ Cola Manual
                                </button>
                            ...
                            {cookieManagerTab === 'auto' && (
                                <div className="space-y-3">
                                    <p className="text-xs text-gray-500">
                                        O sistema vai extrair automaticamente os cookies do seu navegador. <strong>Feche o navegador antes!</strong>
                                    </p>
                                    <div className="flex gap-2 flex-wrap">
                                        {(['chrome', 'firefox', 'edge'] as const).map(b => (
                                            ...
                                            >
                                                {b === 'chrome' ? '🟡' : b === 'firefox' ? '🦊' : '🔵'} {b.charAt(0).toUpperCase() + b.slice(1)}
                                            </button>
                                        ))}
                                    </div>
                                    <button
                                        onClick={handleUpdateFromBrowser}
                                        disabled={cookieUpdating}
                                        className="w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-black text-sm disabled:opacity-60 hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg flex items-center justify-center gap-2"
                                    >
                                        {cookieUpdating ? <><Loader2 size={16} className="animate-spin" /> Extraindo cookies...</> : '🔄 Extrair Cookies do ' + selectedBrowser.charAt(0).toUpperCase() + selectedBrowser.slice(1)}
                                    </button>
                                    <div className="bg-gray-50 rounded-xl p-3 text-xs text-gray-500 space-y-1">
                                        <p className="font-bold text-gray-700">📋 Instruções:</p>
                                        <p>1. Faça login no Instagram, Facebook, YouTube e TikTok no {selectedBrowser.charAt(0).toUpperCase() + selectedBrowser.slice(1)}</p>
                                        <p>2. <strong>Feche completamente o {selectedBrowser.charAt(0).toUpperCase() + selectedBrowser.slice(1)}</strong> (não minimize!)</p>
                                        <p>3. Clique em "Extrair Cookies" acima</p>
                                    </div>
                                </div>
                            )}

                            {cookieManagerTab === 'manual' && (
                                <div className="space-y-3">
                                    <p className="text-xs text-gray-500">
                                        Exporte os cookies com a extensão <strong>Cookie-Editor</strong> (Chrome/Firefox) e cole o JSON aqui.
                                    </p>
                                    <div className="flex gap-2">
                                        {cookiePlatforms.map(p => (
                                            <button
                                                ...
                                                className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all capitalize border-2 ${
                                                    cookiePlatform === p
                                                        ? 'border-indigo-500 bg-indigo-50 text-indigo-700'
                                    ...
                                    <textarea
                                        value={manualCookiesText}
                                        onChange={e => setManualCookiesText(e.target.value)}
                                        rows={5}
                                        placeholder={`Cole aqui o JSON exportado do Cookie-Editor para ${cookiePlatform}.com...`}
                                        className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl text-xs font-mono resize-none focus:border-indigo-400 outline-none transition-all"
                                    />
                                    <button
                                        onClick={handleUpdateManual}
                                        disabled={cookieUpdating || !manualCookiesText.trim()}
                                        className="w-full py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-black text-sm disabled:opacity-60 hover:from-emerald-600 hover:to-teal-700 transition-all shadow-lg flex items-center justify-center gap-2"
                                    >
                                        {cookieUpdating ? <><Loader2 size={16} className="animate-spin" /> Salvando...</> : '💾 Salvar Cookies do ' + cookiePlatform.charAt(0).toUpperCase() + cookiePlatform.slice(1)}
                                    </button>
                                    <div className="bg-blue-50 rounded-xl p-3 text-xs text-blue-700 space-y-1 border border-blue-100">
                                        <p className="font-bold">📋 Como usar Cookie-Editor:</p>
                                        <p>1. Instale a extensão <strong>Cookie-Editor</strong> no Chrome/Firefox</p>
                                        <p>2. Abra o site ({cookiePlatform}.com) e faça login</p>
                                        <p>3. Clique no ícone da extensão → Export → Export as JSON</p>
                                        <p>4. Cole o JSON no campo acima e clique em Salvar</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {cookieMsg && (
                                <div className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-bold ${
                                    cookieMsg.type === 'ok' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-600 border border-red-200'
                                }`}>
                                    {cookieMsg.type === 'ok' ? '✅' : '❌'} {cookieMsg.text}
                                </div>
                            )}
                        </div>
                    </motion.div>
                            ...
                            <div className="p-5 space-y-4 overflow-y-auto flex-1">
                                ...
                                <div>
                                    <div className="flex flex-col gap-2 mb-2">
                                        ...
                                        {/* Campo de Pesquisa de Páginas / Contas */}
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-8 pr-8 py-2 text-xs bg-gray-50 hover:bg-gray-100/70 border border-gray-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-400 transition-all placeholder:text-gray-400 font-medium"
                                            />
                                            {accountSearch && (
                                                <button
                                                    type="button"
                                    ...
                                    <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                                        {totalFilteredAccounts === 0 ? (
                                            <div className="py-6 text-center text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                                ...
                                                <p className="text-xs font-semibold text-gray-500">Nenhuma conta ou página encontrada</p>
                                                <p className="text-[10px] text-gray-400 mt-0.5">Nenhum resultado para "{accountSearch}"</p>
                                            </div>
                                        ) : (
                                            <>
                                            ...
                                            <div className="bg-purple-50 rounded-2xl p-4 space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-2 items-center">
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                            <span className="text-[10px] font-bold text-gray-400 ml-1">Outro:</span>
                                                            <input 
                                                                type="number" 
                                                                min="1" 
                                                                max="50"
                                                                placeholder="Nº"
                                                                value={postsPerDay > 5 ? postsPerDay : ''}
                                                                onChange={(e) => {
                                                                    ...
                                                                    if (!isNaN(n) && n > 0 && n <= 50) {
                                                                        ...
                                                                        setTimeSlots(prev => {
                                                                            const defs = [];
                                                                            for(let i=0; i<n; i++) {
                                                                                let hour = 8 + Math.floor(i * (14 / Math.max(1, n-1)));
                                                                                if(hour > 23) hour = 23;
                                                                                const m = Math.floor(Math.random() * 60).toString().padStart(2, '0');
                                                                                defs.push(`${hour.toString().padStart(2, '0')}:${n > 10 ? m : '00'}`);
                                                                            }
                                                                            let next = [...prev].slice(0, n);
                                                                            for (const d of defs) {
                                                                                if (next.length >= n) break;
                                                                ...
                                                                className={`w-16 h-10 px-2 rounded-xl border font-black text-sm text-center outline-none transition-all ${postsPerDay > 5 ? 'border-purple-600 bg-purple-600 text-white' : 'border-gray-200 bg-white text-gray-600 focus:border-purple-500'}`}
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
