...
const PublicMLVitrinePage: React.FC = () => {
    // Manually extract userId from URL /ml-vitrine/:userId
    const getUserIdFromUrl = () => {
        const parts = window.location.pathname.split('/');
        const index = parts.indexOf('ml-vitrine');
    ...
    const [userName, setUserName] = useState('Minha Vitrine Mercado Livre');
    ...
    const fetchData = async (isFirstLoad = false) => {
        ...
        try {
            ...
            const response = await axios.get(`/api/public/ml-vitrine/${userId || '1'}?keyword=${encodeURIComponent(search)}&page=${currentPage}`);
        ...
        return (
            <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
                <div className="text-center">
                    <Loader2 className="animate-spin mx-auto text-[#FFE600] mb-4" size={48} />
    ...
    const trackClick = async (linkId: number) => {
        try {
            await axios.post('/api/public/ml-vitrine/track', {
                type: 'click',
                linkId,
                userId
            });
    ...
    // Theme logic adapted to ML premium aesthetic
    const themeStyles: any = {
        ...
        'Gradiente Mistico': {
            bg: 'bg-[#0F0C29] bg-gradient-to-br from-[#0C152B] via-[#1F2C4A] to-[#121A30]',
            text: 'text-white',
    ...
    return (
        <div className={`min-h-screen ${theme.bg} pb-32 selection:bg-[#FFE600]/30 overflow-x-hidden relative transition-all duration-1000`} style={{ fontFamily }}>
            ...
            <div className="pt-16 pb-12 px-6 relative z-10">
                <div className="max-w-xl mx-auto text-center">
                    ...
                    <div className="w-24 h-24 mx-auto mb-6 relative">
                        ...
                        <div className={`relative w-full h-full p-1 rounded-full border border-white/10 overflow-hidden shadow-2xl ${theme.bg}`}>
                            {settings?.logo_url ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-[#FFE600]/10 rounded-full">
                                    <ShoppingBag size={40} className="text-[#2D3277]" />
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <p className={`text-xs font-bold max-w-sm mx-auto leading-relaxed italic opacity-80 ${theme.desc}`}>
                        {settings?.description || "✨ Os melhores achadinhos e ofertas selecionadas no Mercado Livre."}
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-xl mx-auto px-6 relative z-20 space-y-8">
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                        {modularLinks.map((link: any) => (
                            ...
                            >
                                ...
                                <span className="relative z-10 flex items-center justify-center gap-3">
                                    {link.title} 
                                    {link.urgency && <TrendingUp className="text-[#FFE600] animate-pulse" size={16} />}
                                </span>
                            </a>
                        ))}
                    </div>
                ...
                <div className="space-y-6">
                    {links.map((link) => (
                        ...
                        >
                            <div className="flex items-center gap-5">
                                <div className="w-28 h-28 rounded-[2.2rem] overflow-hidden flex-shrink-0 border border-white/5 shadow-2xl bg-white flex items-center justify-center">
                                    <img 
                                        src={link.image_url} 
                                        className="w-full h-full object-contain p-2 group-hover:scale-125 transition-transform duration-[2000ms]" 
                                        alt={link.name} 
                                    />
                                </div>
            ...
            <div className={`mt-24 text-center text-[7px] font-black uppercase tracking-[0.8em] pb-12 opacity-20 ${theme.text}`}>
                SISTEMA ELITE ML © 2026 • TODOS OS DIREITOS RESERVADOS
            </div>
        </div>
    );
};

export default PublicMLVitrinePage;
